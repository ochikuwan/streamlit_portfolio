...
default=['A', 'B', 'C', 'D'], key='column_multiselect')
...
y=df_excel[selected_columns[1]],mode='markers'))
            
            fig.update_layout(title=f"{selected_columns[0]}\
                vs {selected_columns[1]}の散布図")
            st.plotly_chart(fig)
...
"C"])
    selected_column = st.selectbox("データを選択", ["A", "B", "C"],
key="data_select")
...
with st.container():
    ...
    analysis_type = st.radio("分析タイプ", ["平均年齢", "都市別⼈数"],
key="analysis_type")
...
key="nested_slider")
    ...
    with col2:
        ...
        with st.container():
            st.write("右側の上部コンテナ")
            option = st.selectbox("オプションを選択", ["オプション1", "オプション\
2", "オプション3"], key="nested_select")
...
with st.expander("グラフを表⽰"):
    fig = go.Figure(data=go.Scatter(x=sales_data['⽇付'], y=sales_data[
    '売上'], mode='lines+markers'))
...
if filtered_data.empty:
    ...
    if analysis_option == "データ概要":
        ...
        product_sales = filtered_data.groupby('商品')[
        '売上'].sum().reset_index()
...
with st.expander("カスタム分析"):
    selected_product = st.selectbox("分析する商品を選択",
    sales_data['商品'].unique())
